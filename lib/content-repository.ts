...
export function serializeProject(row:typeof projects.$inferSelect):StoredProject {
  return {...row,titleEs:row.titleEs||row.title,descriptionEs:row.descriptionEs||row.description,outcomeEs:row.outcomeEs||row.outcome,repo:row.repo||"",mediaUrl:row.mediaUrl||"",mediaType:row.mediaType||"",mediaAlt:row.mediaAlt||"",stack:json(row.stack,[]),details:json(row.details,[]),detailsEs:json(row.detailsEs,[]),media:json(row.media,[])};
}
