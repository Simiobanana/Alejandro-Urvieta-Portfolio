...
function WorldGallery({items,title,lang}:{items:MediaItem[];title:string;lang:Lang}){
 const [active,setActive]=useState(0),item=items[active];
 if(!item)return <div className="world-media-empty"><span>{title}</span></div>;
 const kind=item.type.startsWith("video")?"video":"image";
 return <div className="world-gallery" data-media-kind={kind}><div className="world-media-stage" data-media-kind={kind}><div key={item.id} className="world-media-slide">{kind==="video"?<ProjectVideo key={item.url} item={item} title={title} lang={lang}/>:<img src={item.url} alt={item.alt||title}/>}</div><div className="world-media-veil"/></div>{items.length>1&&<div className="world-gallery-nav" role="group" aria-label={lang==="es"?"Galería":"Gallery"}>{items.map((entry,index)=><button key={entry.id} onClick={()=>setActive(index)} aria-pressed={index===active} aria-label={`${entry.type.startsWith("video")?"Video":lang==="es"?"Imagen":"Image"} ${index+1}`}><span>{String(index+1).padStart(2,"0")}</span><i/></button>)}</div>}</div>;
}
...
export default function WorldView({project,lang,color,onClose}:{project:StoredProject;lang:Lang;color:string;onClose:()=>void}){
 ...
 return <div className="world-overlay" style={{"--world-color":color} as React.CSSProperties}><div className="portal-transition" aria-hidden="true"/><article ref={ref} className="world-view" role="dialog" aria-modal="true" aria-labelledby="world-title">
  ...
  <WorldGallery items={items} title={title} lang={lang}/>
