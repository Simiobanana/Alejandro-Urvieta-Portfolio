...
export default function ImmersiveTree({projects,settings,lang,realm,effects,onOpen}:{projects:StoredProject[];settings:Settings;lang:Lang;realm:Realm;effects:boolean;onOpen:(p:StoredProject)=>void}){
 ...
 useEffect(()=>{
  ...
  const place=()=>{const {width,height}=el.getBoundingClientRect(),w=Math.min(width,height*treeLayout.aspect),h=w/treeLayout.aspect;for(const node of el.querySelectorAll<HTMLElement>('[data-model-anchor]')){const anchor=treeLayout.anchors.find(a=>a.name===node.dataset.modelAnchor);if(anchor){node.style.left=`${(width-w)/2+anchor.x*w}px`;node.style.top=`${(height-h)/2+anchor.y*h}px`;}}};
 ...
 return <section ref={section} className="ygg-scene" id="work" data-realm={realm} data-renderer={renderer} data-markers={markers}>
  ...
  <div className="tree-view-tools" aria-label={lang==="es"?"Opciones del árbol":"Tree options"}>
   ...
   <span aria-hidden="true">·</span><button aria-pressed={renderer!=="webgl"} onClick={()=>chooseRenderer("svg")}>{lang==="es"?"Ligera":"Light"}</button><button aria-pressed={renderer==="webgl"} onClick={()=>chooseRenderer("webgl")}>3D</button>
