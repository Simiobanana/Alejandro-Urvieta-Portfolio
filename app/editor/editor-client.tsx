...
function ProjectPreview({ project, lang, palette }: { project: ProjectForm; lang: "en" | "es"; palette: Palette }) {
  ...
  const x=project.hotspotX??50,y=project.hotspotY??45,color=isHex(project.accentColor)?project.accentColor:palette.violet;
  return <article className="editor-site-preview" style={paletteStyle(palette)}>
    <div className="editor-world-map" aria-label={lang==="es"?"Posición del proyecto en Yggdrasil":"Project position on Yggdrasil"}><i/><span style={{left:`${x}%`,top:`${y}%`,background:color,boxShadow:`0 0 18px ${color}`}}/><small>{title}</small></div>
