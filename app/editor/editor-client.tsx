...
import { sectionLabels, type MediaItem, type SectionItem, type SectionKind, type StoredProject, type StoredSection } from "@/lib/content-types";
...
const emptyProject: ProjectForm = { title: "", titleEs: "", category: "Programming", description: "", descriptionEs: "", outcome: "", outcomeEs: "", details: "", detailsEs: "", stack: "", href: "", repo: "", mediaUrl: "", mediaType: "", mediaAlt: "", media: [], featured: false, published: true, sortOrder: 100, code: "", accent: "cyan" };
...
export default function EditorClient({ userName }: { userName: string }) {
  ...
  async function saveProject(event: FormEvent) {
    ...
    if (!isLink(project.href) || !isLink(project.repo)) { setMessage({ text: "Los enlaces deben usar HTTPS, una ruta local, un ancla, mailto: o tel:.", error: true }); return; }
  ...
  async function saveSettings(event: FormEvent) {
    event.preventDefault(); if (locked) return;
    if ([...Object.values(settings.palette.dark), ...Object.values(settings.palette.light)].some(value => !isHex(value))) { setMessage({ text: "Usa colores hexadecimales de seis dígitos, por ejemplo #31e9ff.", error: true }); return; }
  ...
  return <main className="admin-shell">
    ...
    <Tabs defaultValue="projects" className="admin-tabs"><TabsList className="admin-tablist"><TabsTrigger value="projects"><FolderKanban />Proyectos</TabsTrigger><TabsTrigger value="sections"><LayoutPanelTop />Secciones</TabsTrigger><TabsTrigger value="appearance"><PaletteIcon />Apariencia</TabsTrigger></TabsList>
      <TabsContent value="projects"><div className="admin-grid"><form className="admin-form glass" onSubmit={saveProject}><fieldset disabled={locked} className="editor-fieldset">
        ...
        <div className="field-pair"><Field label="Enfoque"><select value={project.category} onChange={event => updateProject("category", event.target.value)}><option>Programming</option><option>Art &amp; VFX</option><option>Hybrid</option></select></Field><Field label="Orden · menor número primero"><input type="number" required value={project.sortOrder} onChange={event => updateProject("sortOrder", Number(event.target.value))} /></Field></div>
