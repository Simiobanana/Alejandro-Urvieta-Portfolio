 ...
 useEffect(()=>{
  ...
  const boot=async()=>{
   ...
   try{
    ...
    camera.position.set(.4,10,24);camera.lookAt(0,2.6,0);
    ...
    model.traverse(obj=>{if(!(obj instanceof THREE.Mesh))return;const mats=Array.isArray(obj.material)?obj.material:[obj.material];for(const mat of mats){if(!mat.name.startsWith('Waterfall'))continue;mat.onBeforeCompile=(shader:import("three").WebGLProgramParametersWithUniforms)=>{shader.uniforms.waterTime=waterTime;shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nvarying float vWaterY;').replace('#include <begin_vertex>','#include <begin_vertex>\nvWaterY=position.y;');shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nuniform float waterTime; varying float vWaterY;').replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb *= .8 + .2 * sin(vWaterY*12.0 + waterTime*3.5);');};mat.customProgramCacheKey=()=>"astral-waterfall-v1";}});
    ...
    const draw=(time:number)=>{
     ...
     waterTime.value=current.current.effects?time*.001:0;renderer.render(scene,camera);
    };
