...
La página principal utiliza el idioma, día/noche, efectos, categorías, colores y orden del contenido del editor existente. Los proyectos se abren en sus casos completos sin perder el reino seleccionado. El render WebP de esta misma composición se utiliza como alternativa automática si WebGL no está disponible o el navegador solicita ahorro de datos; no se muestra un control manual de vista ligera/3D. Las posiciones X/Y del árbol anterior se conservan en el editor como datos de la versión anterior y no alteran los anclajes de esta escena.

Renderizado WebGL con sombras, entorno de iluminación, transición de cámara a cada reino y modo día/noche. Cada reino muestra proyectos del contenido público existente y enlaza a sus casos completos. Las islas pueden seleccionarse con clic o toque sobre la superficie visible del modelo; volver a seleccionar el reino activo regresa a la vista general del árbol, tanto desde la isla como desde el selector. Arrastrar o desplazar la página no selecciona una isla. El selector permanece disponible para teclado y vista ligera.

Objetivo de 60 fotogramas por segundo con cadencia acumulada y transiciones independientes de la frecuencia de pantalla. Las sombras de la geometría estática se calculan una vez; la resolución se ajusta gradualmente si la cadencia sostenida cae. La escena se pausa fuera de pantalla, con la pestaña oculta o al abrir un proyecto; respeta el control de efectos y movimiento reducido. El control táctil permite desplazamiento vertical: la rotación manual se reserva al ratón. El render WebP es la alternativa si WebGL no funciona. No se garantiza una velocidad idéntica en todos los equipos.

En desarrollo, el canvas expone métricas de cadencia, tiempo de envío CPU y llamadas/triángulos de dibujo mediante atributos `data-*` para comparar cambios. No equivalen a tiempos GPU ni a una medición física en iPhone. La instrumentación se elimina de producción.
